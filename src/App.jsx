...
// Typography helpers — Syne + system fonts
const F = {
  display: "'Syne', system-ui, sans-serif",        // big bold headlines
  body:    "'Syne', system-ui, sans-serif",        // clean body text
  mono:    "'Syne', system-ui, sans-serif",        // labels, stats
};
...
function HomePage({ setPage, setSelectedHackathon, user }) {
  ...
  return (
    <div style={{ background: "#fff", fontFamily: F.body, minHeight: "100vh" }}>
      {/* Header */}
      <div style={{ background: "#fff", borderBottom: `1px solid ${T.border}`, padding: "24px 0" }}>
        <Wrap style={{ padding: "0 28px" }}>
          <div style={{ marginBottom: 24 }}>
            <h1 style={{ fontSize: "32px", fontWeight: 800, color: T.ink, margin: "0 0 8px", fontFamily: F.display }}>Join the world's best online and in-person hackathons</h1>
          </div>

          {/* Search bar */}
          <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
            <div style={{ position: "relative", flex: 1, maxWidth: 500 }}>
              <span style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", fontSize: 18, color: T.ink3 }}>⌕</span>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Find your next hackathon"
                style={{ width: "100%", padding: "12px 16px 12px 48px", border: `1.5px solid ${T.border}`, fontSize: 14, color: T.ink, outline: "none", boxSizing: "border-box", fontFamily: F.body, background: "#fff", borderRadius: 4, transition: "border-color 0.2s" }}
                onFocus={e => e.currentTarget.style.borderColor = T.emerald}
                onBlur={e => e.currentTarget.style.borderColor = T.border} />
            </div>
            <Btn onClick={() => {}} style={{ background: T.teal, color: "#fff", fontWeight: 700 }}>Search</Btn>
          </div>
      ...
      <Wrap style={{ padding: "24px 28px", display: "grid", gridTemplateColumns: "220px 1fr", gap: 32, alignItems: "start" }}>
          ...
          <div style={{ position: "sticky", top: 100 }}>
            ...
            <div style={{ marginBottom: 28 }}>
              <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", marginBottom: 12 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 600, color: T.ink, fontFamily: F.body }}>Match my eligibility</span>
              </label>
            </div>
        ...
        <div>
          {sorted.length === 0 ? (
            <div style={{ textAlign: "center", padding: "80px 24px", background: T.cream, borderRadius: 8 }}>
              ...
              <div style={{ fontSize: 14, color: T.ink3 }}>Try adjusting your search or filters</div>
            </div>
          ) : (
              ...
              {sorted.map((h, idx) => {
                ...
                return (
                  <div key={h.id} onClick={() => { setSelectedHackathon(h); setPage("hackathon-detail"); }}
                    style={{ background: "#fff", border: `1px solid ${T.border}`, borderRadius: 8, overflow: "hidden", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "stretch", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = T.ink3; e.currentTarget.style.boxShadow = "0 4px 12px rgba(0,0,0,0.1)"; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = T.border; e.currentTarget.style.boxShadow = "0 1px 3px rgba(0,0,0,0.05)"; }}>
                    
                    {/* Featured badge */}
                    {idx < 2 && (
                      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 6, background: h.status === "open" ? T.emerald : T.orange, zIndex: 1 }} />
                    )}

                    {/* Thumbnail */}
                    <div style={{ width: 150, height: 150, flexShrink: 0, position: "relative", overflow: "hidden", background: T.cream }}>
                      <img src={h.thumb} alt={h.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      {idx < 2 && (
                        <div style={{ position: "absolute", top: 8, right: 8, background: T.orange, color: "#fff", padding: "4px 10px", borderRadius: 3, fontSize: 10, fontWeight: 700, fontFamily: F.body }}>
                          FEATURED
                        </div>
                      )}
                    </div>
                    ...
                    <div style={{ padding: "16px 18px", flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                      <div>
                        ...
                        <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 8 }}>
                          <div style={{ flex: 1 }}>
                            <h3 style={{ fontSize: 16, fontWeight: 700, color: T.ink, margin: "0 0 4px", lineHeight: 1.3, fontFamily: F.display }}>
                              {h.title}
                            </h3>
                            <p style={{ fontSize: 13, color: T.ink3, margin: 0, fontFamily: F.body }}>
                              by <strong style={{ color: T.ink2 }}>{h.organizer}</strong>
                            </p>
                          </div>
                        ...
                        {/* Dates and details */}
                        <div style={{ fontSize: 12, color: T.ink3, margin: "8px 0 10px", fontFamily: F.body }}>
                          🌐 Online
                        </div>

                        {/* Tags */}
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          {h.tags.slice(0, 3).map(t => <Tag key={t} label={t} />)}
                        </div>
                      </div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 12, paddingTop: 12, borderTop: `1px solid ${T.border}` }}>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 800, color: T.ink, fontFamily: F.display }}>{h.prize}</div>
                          <div style={{ fontSize: 11, color: T.ink3, fontFamily: F.body }}>in prizes</div>
                        </div>
                        <div style={{ textAlign: "right" }}>
                          <div style={{ fontSize: 14, fontWeight: 700, color: T.ink2, fontFamily: F.body }}>{regCount}</div>
                          <div style={{ fontSize: 11, color: T.ink3, fontFamily: F.body }}>participants</div>
                        </div>
                      </div>
                    </div>
                  </div>
